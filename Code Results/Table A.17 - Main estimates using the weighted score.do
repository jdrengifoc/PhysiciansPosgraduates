	...
	Replication package
	Table A.17.  Main results using the weighted score without and with controls
...
cap log close
log using "Logs/Table A17.smcl", replace

********************************************************************************

	use 		"Data\base_hospital_cohorte_bebe", replace

	*Denominator is the number of physicians per municipality in 2012
	cap drop _merge 
	merge m:1 codmundep using "Data\Anonimizada\Weights_2012.dta", keep(3)
	drop _merge
	
	* We drop municipalities with 1 hospital
	...
	*Creating hospital covars (bw and prematurity 3 years before)
	...
	gen 	w_docinic=round(days_coinc)*n_docs
	...
	egen Total_physic=rowmean(medico_*)
	bys id_nacido: egen physic_cohorte=total(n_docs)
	bys id_nacido: egen min_date_sso=min(p_fechin_obs_ave)
	bys id_nacido: egen max_date_sso=max(p_fechfi_obs_ave)
	format min_date_sso max_date_sso %td
	gen exposicion=min(max_date_sso,fechanto)-max(v_fconcep,min_date_sso)
	gen dias_gestacion=semanas_gestacion*7
	
	collapse (mean) ave_prom_4 comp1_health ave_comp1 ave_prom_health ave_ecaes_med_administracion ave_ecaes_med_spublica ave_ecaes_med_reading ave_ecaes_med_quantitative (max) exposicion dias_gestacion physic_cohorte dummy_female dummy_top dummy_public Total_physic (sum) n_docs [fw=w_doc],by($c_b pop100 p_drawm dummy_1hosp area $c_h $outcomes embarazos_madre d_area_metro id_nacido id_h year codigo_tot)

	gen ponderador=physic_cohorte/Total_physic
	replace ponderador=. if ponderador>1
	...
	*Estandarizar los puntajes
	...
	mat list table1 
	putexcel set "Results\Tables paper matrices replication.xls", sheet("Raw Table A17") modify
