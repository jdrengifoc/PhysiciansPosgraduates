	...
	Replication package
	Table A.15.  Interaction between cohort scores and program scores
...
********************************************************************************
	...
	*Creating hospital covars (bw and prematurity 3 years before)
	...
	collapse (mean) ave_prog ave_prom_4 comp1_health ave_comp1 ave_prom_health ave_ecaes_med_administracion ave_ecaes_med_spublica ave_ecaes_med_reading ave_ecaes_med_quantitative (max) dummy_female dummy_top dummy_public [fw=w_doc],by($c_b pop100 p_drawm dummy_1hosp area $c_h $outcomes embarazos_madre d_area_metro id_nacido id_h year codigo_tot)
	...
	*Estandarizar los puntajes
	...
	sum     ave_prog if tagc==1 & $p_res
	replace ave_prog=(ave_prog-r(mean))/r(sd)
	...
	foreach var in unhealthy $outcomes {
		...
		reghdfe `var' c.ave_prom_health##c.ave_prog  $res2
		mat 	table1[1,`c']  = _b[ave_prom_health]
		mat 	table1[2 ,`c'] = _se[ave_prom_health]
		
		mat 	table1[4,`c']  = _b[ave_prog]
		mat 	table1[5 ,`c'] = _se[ave_prog]
		
		
		mat 	table1[7,`c']  = _b[c.ave_prom_health#c.ave_prog]
		mat 	table1[8 ,`c'] = _se[c.ave_prom_health#c.ave_prog]
			
		qui tab codigo_tot if e(sample)==1
		mat table1[11,`c'] = e(N) // obs
						
		sum `var' if e(sample)==1
		mat table1[10,`c']  = r(mean)
		mat table1[3,`c'] = table1[1,`c']/table1[10,`c']
		mat table1[6,`c'] = table1[4,`c']/table1[10,`c']
		mat table1[9,`c'] = table1[7,`c']/table1[10,`c']
	...
	mat list table1 
	putexcel set "Results\Tables paper matrices replication.xls", sheet("Raw Table A15") modify
