	...
	Replication package
	Table A.14. Main estimates without, with dummy and continuous controls
...
cap log close
log using "Logs/Table A14.smcl", replace
...
********************************************************************************
	...
	*Creating hospital covars (bw and prematurity 3 years before)
	...
	foreach var in unhealthy $outcomes  {
		...
		reghdfe `var' `x' $res1_cn
		mat 	table1[7,`c']  = _b[`x']
		mat 	table1[8 ,`c'] = _se[`x']
	...
	mat list table1 
	putexcel set "Results\Tables paper matrices replication.xls", sheet("Raw Table A14") modify	
