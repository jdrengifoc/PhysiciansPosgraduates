	...
	Replication package
	Figure A.11.  Main results using covarianze index (Anderson, 2006)
...
cap log close
log using "Logs/Table A11.smcl", replace
...
********************************************************************************
	...
	*Standardize scores
	...
	sum     unhealthy
	gen     unhealthy_stan = (unhealthy - r(mean))/r(sd)
	
	*Standardize outcome variables
	foreach var in $outcomes {
	sum `var'
	gen `var'_stan = (`var' - r(mean))/r(sd)
	}
	
	*New outcome using inverse covariance index
	icw_index bpn_stan nacer_menos_37_stan apgar1menor7_stan, gen(unhealthy_index)
	...
	local c=1
	foreach var in unhealthy_index unhealthy_stan {
		...
		mat table1[7,`c'] = e(N) // obs
						
		sum `var' if e(sample)==1
		mat table1[3,`c']  = table1[1,`c']/r(sd)
		mat table1[6,`c']  = table1[4,`c']/r(sd)
	...
	mat list table1 
	putexcel set "Results\Tables paper matrices replication.xls", sheet("Raw Table A11") modify
