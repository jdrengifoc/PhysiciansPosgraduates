	...
	Replication package
	Table 4. Main estimates using all sample and average score
...
cap log close
log using "Logs/Table 4 - Table A10 - Figure A8.smcl", replace
...
********************************************************************************
	...
	*Creating hospital covars (bw and prematurity 3 years before)
	...
	matlist table1	
	putexcel set "Results\Tables paper matrices replication.xls", sheet("Raw  Table 4") modify
	...
	mat list table1 
	putexcel set "Results\Tables paper matrices replication.xls", sheet("Raw Table A10") modify
	putexcel B2=mat(table1)	
...
tw		(rcap 	lb ub x if type==1,lc(black)msize(zero)lw(medium)) ///
		...
		,legend(c(4) position(6) region(lc(white)) nobox order(2 "With Controls" 4 "W/o Controls" 1 "Average" 5 "Principal Comp.")) ///
		graphr(c(white)) xtitle("") xlabel(1(1)4,valuelabel)yline(0.1,lc(gs15))  ///
		yline(0,lc(red)lp(S)lw(thin)) ylabel(-0.2(0.1)0.1,angle(h))
		
graph export "Results\Figure A8.png",replace wid(1446) hei(1050)
