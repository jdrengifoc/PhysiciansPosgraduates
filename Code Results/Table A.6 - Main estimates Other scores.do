	...
	Replication package
	TABLE A.6.  Main estimates using all the areas tested in the SABER PRO
...
cap log close
log using "Logs/Table A6.smcl", replace
...
********************************************************************************
	...
	*Creating hospital covars (bw and prematurity 3 years before)
	...
	global      skills   ave_prom_health ave_ecaes_med_administracion ave_ecaes_med_spublica ave_prom_acad ave_ecaes_med_reading ave_ecaes_med_quantitative  
	...
	collapse (mean) ave_prom_4 comp1_health ave_comp1 $skills (max) dummy_female dummy_top dummy_public [fw=w_doc],by($c_b pop100 p_drawm dummy_1hosp area $c_h $outcomes embarazos_madre d_area_metro id_nacido id_h year codigo_tot)
	...
	* Standardized scores
	...
	sum     ave_prom_acad if tagc==1 & $p_res
	replace ave_prom_acad=(ave_prom_acad-r(mean))/r(sd)
	sum     ave_ecaes_med_reading if tagc==1 & $p_res
	replace ave_ecaes_med_reading=(ave_ecaes_med_reading-r(mean))/r(sd)
	sum     ave_ecaes_med_quantitative if tagc==1 & $p_res
	replace ave_ecaes_med_quantitative=(ave_ecaes_med_reading-r(mean))/r(sd)
	...
	foreach var in unhealthy  {
		...
		foreach x in ave_prom_4 ave_prom_health ave_ecaes_med_administracion ave_ecaes_med_spublica ave_prom_acad ave_ecaes_med_reading ave_ecaes_med_quantitative   {
	...
	mat list table1 
	putexcel set "Results\Tables paper matrices replication.xls", sheet("Raw Table A6") modify
